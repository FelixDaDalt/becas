...
export class ColegioService {
  ...
  private colegioDetalleSubject = new BehaviorSubject<ColegioDetalle|null>(null)
  colegioDetalle$ = this.colegioDetalleSubject.asObservable().pipe(shareReplay(1))
  ...
  private getColegios(){
    return this.http.get(`${environment.apiUrl}${environment.endpoint.colegio.listado}`).pipe(
      map((respuesta:any) => respuesta.data))
  }

  obtenerColegios(){
    this.getColegios().pipe(take(1)).subscribe(
        colegios=>{
  ...
  suspenderColegio(idColegio:number){
    this.http.put(`${environment.apiUrl}${environment.endpoint.colegio.suspender}?idColegio=${idColegio}`,null).pipe(
      take(1),
      tap((respuesta: any) => {
  ...
  altaColegio(nuevoColegio: any): Observable<any> {
    return this.http.post(`${environment.apiUrl}${environment.endpoint.colegio.alta}`, nuevoColegio).pipe(
      take(1),
      tap((respuesta: any) => {
        this.toast.success(respuesta.mensaje);
  ...
  private getColegio(){
    return this.http.get(`${environment.apiUrl}${environment.endpoint.colegio.obtener}`).pipe(
      map((respuesta:any) => respuesta.data))
  }

  obtenerColegio(){
    this.getColegio().pipe(take(1)).subscribe(
      colegio=>this.colegioSubject.next(colegio))
  }

  private getDetalle(idColegio:number | string){
    return this.http.get(`${environment.apiUrl}${environment.endpoint.colegio.detalle}?idColegio=${idColegio}`).pipe(
      map((respuesta:any) => respuesta.data))

  }

  obtenerDetalle(idColegio:number | string){
    return this.getDetalle(idColegio).pipe(
      tap(colegio=>this.colegioDetalleSubject.next(colegio)),
      take(1))
  }
