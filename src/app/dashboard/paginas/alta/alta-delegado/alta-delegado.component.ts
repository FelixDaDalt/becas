...
@Component({
  selector: 'app-alta-delegado',
  templateUrl: './alta-delegado.component.html',
  styleUrls: ['./alta-delegado.component.css']
})
export class AltaDelegadoComponent{
  ...
  constructor(private fb: FormBuilder,
    ...
    private delagadoService:DelegadoService,
  ...
  submitForm() {
    if (this.wizardForm.valid) {
      ...
      this.delagadoService.altaDelegado(formData).subscribe(respuesta=>{
        this.router.navigate(['../delegados'], {relativeTo:this.activeRoute});
      })
    }else{
  ...
  private estructurarFormulario() {
    ...
    const delegado = this.wizardForm.get('delegado')?.value;
