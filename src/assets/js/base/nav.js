  ...
  _matchUrlByMenu(selector) {
    ...
    document.querySelectorAll(selector + ' a').forEach((el) => {
      if (el && el.getAttribute('data-href')) {
        ...
        // Añadir evento clic para llamar a la función `navegar`
        el.addEventListener('click', (event) => {
          event.preventDefault(); // Evitar la navegación por defecto
          window.navegar(hrefData, el); // Pasar la ruta y el elemento al método `navegar`
        });


      if (urlWithoutBackslash.includes(hrefData)) {
